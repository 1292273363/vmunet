...
def load_config_class(config_ref=None, default_module='configs.config_setting'):
    """Load a config class from either a module path or a config filename."""
    if config_ref is None:
        ...
        if config_path.suffix == '.py':
            ...
            module_name = f'configs.{stem}' if config_path.parent in {Path('.'), Path('configs')} else '.'.join(config_path.with_suffix('').parts)
        elif '.' in config_ref:
            module_name = config_ref
        else:
            module_name = f'configs.{config_ref}'

    return importlib.import_module(module_name).setting_config



def get_optimizer(config, model):
    assert config.opt in ['Adadelta', 'Adagrad', 'Adam', 'AdamW', 'Adamax', 'ASGD', 'RMSprop', 'Rprop', 'SGD'], 'Unsupported optimizer!'
    params = _build_optimizer_param_groups(config, model)
...
def _qualify_param_name(prefix, name):
    return f'{prefix}.{name}' if prefix else name


def _collect_no_weight_decay_names(model):
    ...
    for module_name, module in model.named_modules():
        if isinstance(module, norm_types):
            for param_name, _ in module.named_parameters(recurse=False):
                no_decay_names.add(_qualify_param_name(module_name, param_name))
        ...
        if hasattr(module, 'no_weight_decay_keywords'):
            no_decay_keywords.update(module.no_weight_decay_keywords())

    for param_name, _ in model.named_parameters():
        if param_name.endswith('.bias') or param_name == 'bias':
            no_decay_names.add(param_name)
        if 'gamma' in param_name:
            no_decay_names.add(param_name)
        if any(keyword in param_name for keyword in no_decay_keywords):
            no_decay_names.add(param_name)

    return no_decay_names


def _build_optimizer_param_groups(config, model):
    no_decay_names = _collect_no_weight_decay_names(model)
    decay_params = []
    no_decay_params = []

    for param_name, param in _unwrap_model(model).named_parameters():
        if not param.requires_grad:
            continue
        if param_name in no_decay_names:
